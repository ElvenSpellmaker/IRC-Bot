...
class SayRedirector
{
	...
	{
		$this->connection = $this->bot->getConnection();
	...
	/**
	 * Sends PRIVMSG to toNick whether it's a channel or a PM.
	 *
	 * @param string $toNick
	 */
	protected function sendToBot($toNick, $messages)
	{
		...
		if(strpos($toNick, '#') === FALSE)
			$this->sendData( $beginningMsg .'These messages originate from '. $this->channel );
		
		foreach( $messages as $msg )
			$this->sendData( $beginningMsg . $msg );
	}
